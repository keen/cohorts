...
function generateCohortQueryMatrix(dm, fn) {
  ...
  Keen.utils.each(dm, function(cohort, i){
    matrix[i] = new Array();
    // console.log('Cohort signed up', cohort[0].start);
    Keen.utils.each(cohort, function(interval, j){
      var context = {
...
function fetchCohortDatasets(client, cohorts, callback) {
  ...
  var inProgress = 0;
  Keen.utils.each(cohorts, function(cohort, i){
    ...
    client.run(cohort, function(err, cohortResult){
      ...
      Keen.utils.each(cohortResult, function(res, j){
...
function formatCohortDatasets(dataset){
  ...
  Keen.utils.each(dataset.matrix, function(row, i) {
